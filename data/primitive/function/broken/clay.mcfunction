...
execute at @s as @e[limit=1,sort=nearest,type=minecraft:armor_stand,tag=break_counter] if score @s break_counter matches 64.. run kill @e[limit=1,sort=nearest,type=minecraft:interaction,tag=clay_blob_interaction,distance=..1]
