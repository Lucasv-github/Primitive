execute at @s align xyz positioned ~0.5 ~ ~0.5 unless entity @e[limit=1,sort=nearest,type=minecraft:armor_stand,tag=tree_root,distance=..1] run function primitive:tree/count
...
execute at @s as @e[limit=1,sort=nearest,type=minecraft:armor_stand,tag=tree_root] if score @s tree_count matches 0 at @s run function primitive:tree/destroy
